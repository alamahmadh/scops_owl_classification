...
def _conv_bn_relu(nb_filter, nb_row, nb_col, subsample=(1, 1)):
    def f(input):
        ...
        norm = BatchNormalization(mode=0, axis=3)(conv)
...
def _shortcut(input, residual):
    ...
    if stride_width > 1 or stride_height > 1 or not equal_channels:
        shortcut = Conv2D(nb_filter=residual._keras_shape[3], nb_row=1, nb_col=1,
...
class ResNetBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions):
        ...
        # Classifier block
        pool2 = AveragePooling2D(pool_size=(block._keras_shape[1], block._keras_shape[2]), strides=(1, 1))(block)
...
def main():
    model = ResNetBuilder.build_resnet_18((253, 512, 1), 20)
