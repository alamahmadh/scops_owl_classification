'''
A set of tools as well as data generator for real-time data augmentation 
on the resulting image data which are converted on fly from the wav files. 
These codes are slightly modified version of sound data generator by 
...
def load_wav_as_mfcc(fname, target_size=None, class_dir=None):
    
    (fs, signal) = read_wave_file(fname)

    if class_dir:
        signal = same_class_augmentation(signal, class_dir)

    mfcc = librosa.feature.mfcc(signal, fs, n_mfcc=target_size[0], hop_length=128)
...
def load_wav_as_mfcc_delta(fname, target_size=None, class_dir=None):
...
def load_wav_as_spectrogram(fname, target_size=None, class_dir=None):
    
    (fs, signal) = read_wave_file(fname)

    spectrogram = wave_to_sample_spectrogram(signal, fs)

    if target_size:
        spectrogram = skimage.transform.resize(spectrogram, target_size)

    spectrogram = spectrogram.reshape((spectrogram.shape[0],
                                       spectrogram.shape[1], 1))
    return spectrogram

def load_wav_as_melspectrogram(fname, target_size=None, class_dir=None):
    ...
    melspectrogram = wave_to_sample_melspectrogram(signal, fs)

    if target_size:
        melspectrogram = skimage.transform.resize(melspectrogram, target_size)

    melspectrogram = melspectrogram.reshape((melspectrogram.shape[0],
                                             melspectrogram.shape[1], 1))
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        if audio_mode not in {'melspectrogram', 'spectrogram', 'mfcc', 'mfcc_delta'}:
            raise ValueError('Invalid audio mode:', audio_mode,
                             '; expected "melspectrogram", "spectrogram", "mfcc" or "mfcc_delta".')
        self.audio_mode = audio_mode
        self.dim_ordering = tf.keras.backend.image_data_format()
        if self.audio_mode == 'mfcc_delta':
            self.image_shape = self.target_size + (4,)
        elif self.audio_mode in {'melspectrogram', 'mfcc', 'spectrogram'}:
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            ...
            if self.audio_mode == 'melspectrogram':
                x = load_wav_as_melspectrogram(os.path.join(self.directory, fname),
                                               target_size=self.target_size,
                                               class_dir=class_dir)
            ...
            if self.image_data_generator.time_shift:
                x = time_shift_spectrogram(x)
            if self.image_data_generator.pitch_shift:
                x = pitch_shift_spectrogram(x)
